...
def solution(picks, minerals):
    ...
    if sum(picks) >= needs:
        combination = [p for p in product(range(picks[0]+1), range(picks[1]+1), range(picks[2]+1)) if sum(p)==needs]
    else:
        combination = [p for p in product(range(picks[0]+1), range(picks[1]+1), range(picks[2]+1)) if sum(p)==sum(picks)]
